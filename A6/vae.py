...
class CVAE(nn.Module):
    def __init__(self, input_size, num_classes=10, latent_size=15):
        ...
        # Replace "pass" statement with your code
        self.hidden_dim = 128
        print(input_size+self.num_classes)
        self.encoder = nn.Sequential(
                nn.Flatten(),
                nn.Linear(input_size+self.num_classes, self.hidden_dim),
                nn.ReLU(),
                nn.Linear(self.hidden_dim, self.hidden_dim),
                nn.ReLU(),
                nn.Linear(self.hidden_dim, self.hidden_dim),
    ...
    def forward(self, x, c):
        ...
        # Replace "pass" statement with your code
        N, _, _, _ = x.shape
        mu = self.mu_layer(self.encoder(torch.cat((x.reshape(N, -1), c), dim=1)))
        logvar = self.logvar_layer(self.encoder(torch.cat((x.reshape(N, -1), c), dim=1)))
        z = reparametrize(mu, logvar)
...
def reparametrize(mu, logvar):
    ...
    ################################################################################################
    # Replace "pass" statement with your code
    epsilon = torch.randn(mu.shape, dtype=mu.dtype, device=mu.device)
    z = torch.exp(logvar)**0.5 * epsilon + mu
    ################################################################################################
    #                              END OF YOUR CODE                                                #
    ################################################################################################
    return z
...
def loss_function(x_hat, x, mu, logvar):
    ...
    N, _, H, W = x.shape
    kl_divergence = 0
    kl_divergence = -0.5 * torch.sum(torch.sum((1 + logvar - mu**2 - torch.exp(logvar)), dim=1), dim=0)
