...
class DeepConvNet(object):
  ...
  def __init__(self, input_dims=(3, 32, 32),
    ...
    for i, num_filter in enumerate(num_filters):
      ...
      if self.batchnorm:
        self.params['gamma{}'.format(i)] = 0.01 * torch.randn(num_filter, dtype=dtype, device=device)
      ...
      if weight_scale == 'kaiming':
        self.params['W{}'.format(i)] = kaiming_initializer(num_filter, pred_filters, K=filter_size, relu=True, dtype=dtype, device=device)
      else:
    ...
    i += 1
    if weight_scale == 'kaiming':
        self.params['W{}'.format(i)] = kaiming_initializer(num_filter*H_out*W_out, num_classes,relu=False, dtype=dtype, device=device)
    else:
