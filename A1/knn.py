...
def compute_distances_no_loops(x_train, x_test):
  ...
  dists = torch.sum(x_train_flat**2, dim=1, keepdim=True) \
          - 2 * torch.matmul(x_train_flat, x_test_flat.transpose(0, 1)) \
          + torch.sum(x_test_flat**2, dim=1, keepdim=True).transpose(0, 1)
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
  return dists
...
def predict_labels(dists, y_train, k=1):
  ...
  _, nearest_indices = torch.topk(dists, k=k, largest=False, dim=0)  

  # Count occurrences of each label among the nearest neighbors
  for j in range(num_test):
      nearest_labels = y_train[nearest_indices[:, j]]  # Labels of the k nearest neighbors
      unique_labels, counts = torch.unique(nearest_labels, return_counts=True)
      
      # Find the majority label (in case of tie, choose the smallest label)
      majority_label = unique_labels[counts.argmax()]

      y_pred[j] = majority_label
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
  return y_pred
...
def knn_cross_validate(x_train, y_train, num_folds=5, k_choices=None):
  ...
  for k in k_choices:
    ...
    for fold in range(num_folds):
      # Use all but the current fold as training data
      ...
      # Calculate the accuracy with current training and validation data
