...
def mutate_tensor(x, indices, values):
  ...
  #############################################################################
  # Replace "pass" statement with your code
  assert len(indices) == len(values), "Number of indices must match number of values"
...
def count_tensor_elements(x):
  ...
  if len(tensor_shape) == 1:
    ...
    num_elements = x.shape[0]
    for _ in tensor_shape[1:]:
      num_elements = num_elements * _
  #############################################################################
  #                            END OF YOUR CODE                               #
  #############################################################################
  return num_elements
...
def zero_row_min(x):
  ...
  #############################################################################
  # Replace "pass" statement with your code
  row_min_vals, idx1 = x.min(dim=1)
  idx0 = range(x.shape[0])
  y = x.clone()
  y[idx0, idx1] = 0
  #############################################################################
  #                            END OF YOUR CODE                               #
...
def normalize_columns(x):
  ...
  squared = (x - mu) ** 2
  variance = torch.sum(squared, dim=0) / (x.shape[0] - 1)
